...
- **شريط «آخر النتائج»:** يظهر في **خطوة اختيار المسار** فقط (`#track-feed-panel`)؛ كل سطر = اسم المستخدم + التخصص صاحب **المرتبة الأولى** (أعلى نسبة في النتيجة، عادة 100%) وليس اختياراً يدوياً. التخزين المحلي `uqu_orientation_track_feed_v1`؛ مع Supabase تُستدعى `track_feed_recent` في `sql/supabase-setup.sql` وتُعرَض الأسماء مع `major_rank_1` مُعرَّفاً إلى الاسم من السجل. `trackFeedRecorded` يمنع تكرار السطر عند إعادة `renderResults` لنفس الجلسة.
