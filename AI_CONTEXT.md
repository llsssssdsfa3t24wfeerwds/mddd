...
- **شريط «آخر النتائج»:** يظهر في الخطوات **0–2** (التسجيل، المسار، الاستبيان) لا في صفحة النتائج؛ كل سطر = الاسم + التخصص الأول (أعلى نسبة). محلياً `uqu_orientation_track_feed_v1` مع `emailKey`؛ `trackFeedRecorded` يمنع التكرار في نفس الجلسة. استجابة RPC قد تُلفّ داخل مصفوفة — `normalizeTrackFeedRpcPayload` يفكّ التداخل؛ مع Supabase وبدون صفوف يُعرض نص داخل اللوحة. `normalizeRpcStatsResponse` يفكّ التداخل لـ `major_popularity_stats`.
